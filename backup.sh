...
getFinalStatus() {
    if [[ "${LOG_TYPE}" == "dryrun" ]]; then
        FINAL_STATUS="TESTE REALIZADO COM SUCESSO!"
        LOG_STATUS="OK"
        EXIT=0
    elif [[ $VM_OK == 1 ]] && [[ $VM_FAILED == 0 ]] && [[ $VMDK_FAILED == 0 ]]; then
        FINAL_STATUS="BACKUP REALIZADO COM SUCESSO!"
        LOG_STATUS="OK"
        EXIT=0
    elif [[ $VM_OK == 1 ]] && [[ $VM_FAILED == 0 ]] && [[ $VMDK_FAILED == 1 ]]; then
        FINAL_STATUS="ERRO: All VMs backed up, but some disk(s) failed!"
        LOG_STATUS="WARNING"
        EXIT=3
    elif [[ $VM_OK == 1 ]] && [[ $VM_FAILED == 1 ]] && [[ $VMDK_FAILED == 0 ]]; then
        FINAL_STATUS="ERRO: Only some of the VMs backed up!"
        LOG_STATUS="ERROR"
        EXIT=4
    elif [[ $VM_OK == 1 ]] && [[ $VM_FAILED == 1 ]] && [[ $VMDK_FAILED == 1 ]]; then
        FINAL_STATUS="ERRO: Only some of the VMs backed up, and some disk(s) failed!"
        LOG_STATUS="ERROR"
        EXIT=5
    elif [[ $VM_OK == 0 ]] && [[ $VM_FAILED == 1 ]]; then # $VMDK_FAILED doesn't matter in this case
        FINAL_STATUS="ERRO: All VMs failed!"
        LOG_STATUS="ERROR"
        EXIT=6
    elif [[ $VM_OK == 0 ]] && [[ $VM_FAILED == 0 ]] && [[ $VMDK_FAILED == 0 ]]; then
        FINAL_STATUS="ERRO: No VMs backed up!"
        LOG_STATUS="ERROR"
        EXIT=7
    elif [[ $VM_OK == 0 ]] && [[ $VM_FAILED == 0 ]] && [[ $VMDK_FAILED == 1 ]]; then
        FINAL_STATUS="ERRO: All VMs experienced at least a partial failure!"
        LOG_STATUS="ERROR"
        EXIT=8
    fi
...
buildHeaders() {
    ...
    echo -ne "Subject: CLIENTE [VMS] [ESXI] ${FINAL_STATUS}\r\n" >> "${EMAIL_LOG_HEADER}"
